...
<head>
    ...
    <!-----SEO------>
    @yield('home-description')
    @yield('product-description')
    <link rel="icon" type="image/png" href="{{ asset('storage/' . $siteSettings->favicon) }}" sizes="32x32">
    <!-----SEO------>
    <!------------Linkler--------------->
    ...
    <link rel="stylesheet" href="{{ asset('front/css/prod.css') }}" />
    <link rel="stylesheet" href="{{ asset('front/css/slider.css') }}" />
    <link rel="stylesheet" href="{{ asset('front/css/navmenu.css') }}" />
    <link rel="stylesheet" href="{{ asset('front/css/galeri.css') }}" />
...
<body>
            ...
            <div class="row">
                <div class="col-md-9">
                    <div class="top-contact">
                        ...
                        <span>+{{ $siteSettings->phone_number }}</span>
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div class="social-icons">
                        @if ($siteSettings->instagram)
                            <a href="{{ $siteSettings->instagram }}" target="_blank"><i class="fa fa-instagram"></i></a>
                        @endif
                        @if ($siteSettings->twitter)
                            <a href="{{ $siteSettings->twitter }}" target="_blank"><i class="fab fa-twitter"></i></a>
                        @endif
                        @if ($siteSettings->pinterest)
                            <a href="{{ $siteSettings->pinterest }}" target="_blank"><i class="fa fa-pinterest"></i></a>
                        @endif
    ...
    <nav class="navbar navbar-expand-lg navbar-background">
        <a class="navbar-brand mr-5 ml-3" href="/"><img src="{{ asset('storage/' . $siteSettings->logo) }}"
                alt="{{$siteSettings->site_title}} logo" style="max-width: 100px;">
        </a>
        <button style="box-shadow: none; border: none; color: #b60808;" class="navbar-toggler" type="button"
        ...
        <div class="menu-text collapse navbar-collapse" id="navbarSupportedContent">
            <!-- Çarpı butonu -->
            <button class="btn btn-close-menu" aria-label="Close">
            ...
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('gallery.show') }}"> Galeri </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{route('about.index')}}"> Hakkımızda </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{route('contact.home')}}"> İletişim </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link ml-3" href="{{ route('offer.view') }}"
                        style=" background: #c80707; color:white"> Teklif Al </a>
                </li>

                <li class="nav-item">
                    <a class="nav-link ml-3" href="{{ asset('/uploads/catalogmosaiclamp.pdf') }}"
                        style=" background: #c80707; color:white" download> Kataloğu İndir
                    </a>
                </li>
            </ul>
            ...
            <form action="{{ route('search') }}" class="search-form">
                <div class="search-box">
                    ...
                    <input type="text" name="q" class="input-search" placeholder="  Ürün Ara... ">
                </div>
            </form>
