...
$(document).ready(function () {
  ...
  // Tema çerezi yoksa veya boşsa, varsayılan temayı kullan
  const currentTheme = themeCookie || defaultTheme;

  // Temayı çerezden okuyarak HTML etiketinin class'ına ekle
  document.documentElement.className = currentTheme;

  // İkonu güncelle
  const icon = $(".mode-icon ion-icon");
  icon.attr("name", currentTheme === 'light-theme' ? 'sunny-sharp' : 'moon-sharp');
});
...
$(".dark-mode-icon").on("click", function () {
  ...
  // İkonu güncelle
  const icon = $(".mode-icon ion-icon");
  icon.attr("name", newTheme === 'light-theme' ? 'sunny-sharp' : 'moon-sharp');

  // Tema tercihini çereze kaydet
